...
class MSFBCNN(nn.Module):
	def __init__(self,input_dim,FT=10):
		...
		self.conv1b = nn.Conv2d(1, self.FT, (1, 17), padding = (0,8),bias=False)
		self.conv1c = nn.Conv2d(1, self.FT, (1, 11), padding = (0,5),bias=False)
		self.conv1d = nn.Conv2d(1, self.FT, (1, 7), padding = (0,3),bias=False)

		self.batchnorm1 = nn.BatchNorm2d(4*self.FT, affine=False)
		
		# Spatial convolution
		self.conv2 = nn.Conv2d(4*self.FT, self.FS, (self.C,1),padding=(0,0),groups=1,bias=False)
		self.batchnorm2 = nn.BatchNorm2d(self.FS, affine=False)
